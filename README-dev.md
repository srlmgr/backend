...
[![Build Status](https://img.shields.io/github/checks-status/srlmgr/backend/main?color=black&style=for-the-badge&logo=github)][github-actions]
...
```sh
git clone https://github.com/srlmgr/backend
cd backend
```
...
After cloning the repository you already changed into the project dir backend.
...
This section will guide you to setup a fully-functional local copy of `backend`
...
from [`backend's` Makefile][makefile-file] (most of which are
OS-independent)!

### Installing dependencies

To install all dependencies associated with `backend`, run the
command
...
Tests in `backend` are classified as _fast_ and _slow_ - depending
...
`backend` is powered by a template generated using [`cookiecutter-go`][cookiecutter-go-link] which is based on [`go-template`][go-template-link]
