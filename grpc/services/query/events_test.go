...
func seedEvent(
	...
	seasonID, pointSystemID, trackLayoutID int32,
	name string,
	sequenceNo int32,
) *models.Event {
	t.Helper()
...
func TestListEventsReturnsAll(t *testing.T) {
	...
	event1 := seedEvent(t, repo, season.ID, pointSystem.ID, layout1.ID, "Round 1", 1)
	event2 := seedEvent(t, repo, season.ID, pointSystem.ID, layout2.ID, "Round 2", 2)
...
func TestListEventsBySeasonID(t *testing.T) {
	...
	event1 := seedEvent(t, repo, season1.ID, pointSystem.ID, layout.ID, "Round 1", 1)
	seedEvent(t, repo, season2.ID, pointSystem.ID, layout.ID, "Round 1 S2", 2)
...
func TestGetEventSuccess(t *testing.T) {
	...
	track := seedTrack(t, repo, "Daytona")
	layout := seedTrackLayout(t, repo, track.ID, "Full Circuit")
	event := seedEvent(t, repo, season.ID, pointSystem.ID, layout.ID, "Round 1", 1)
