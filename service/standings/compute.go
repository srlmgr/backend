...
type (
	...
	// SkipModeType defines how to handle skipped events when computing standings
	SkipModeType          int
...
func ParseSkipMode(value string) (SkipModeType, error) {
	switch value {
	case "always":
		return SkipModeAlways, nil
	case "never":
		return SkipModeNever, nil
	case "when_applicable":
		return SkipModeWhenApplicable, nil
	default:
		return 0, fmt.Errorf("invalid skip mode: %s", value)
	}
}
