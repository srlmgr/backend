...
func TestAssignCarModelToCarClassSuccess(t *testing.T) {
	svc, repo := newDBBackedTestService(t)
	ctx := authn.AddPrincipal(context.Background(), &authn.Principal{Name: testUserTester})

	cc := seedCarClass(t, repo, "GT3")
	cm := seedCarManufacturer(t, repo, "Porsche")
	cb := seedCarBrand(t, repo, cm.ID, "911")
	mod := seedCarModel(t, repo, cb.ID, "GT3 RS")

	resp, err := svc.AssignCarModelToCarClass(
		ctx,
		connect.NewRequest(&v1.AssignCarModelToCarClassRequest{
...
func TestAssignCarModelToCarClassTwiceIsNotAnError(t *testing.T) {
	...
	cb := seedCarBrand(t, repo, cm.ID, "488")
	mod := seedCarModel(t, repo, cb.ID, "488 GT3")

	_, err := svc.AssignCarModelToCarClass(
		context.Background(),
		connect.NewRequest(&v1.AssignCarModelToCarClassRequest{
			CarClassId: uint32(cc.ID),
			CarModelId: uint32(mod.ID),
