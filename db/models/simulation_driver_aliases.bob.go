...
var SimulationDriverAliases = psql.NewTablex[*SimulationDriverAlias, SimulationDriverAliasSlice, *SimulationDriverAliasSetter]("", "simulation_driver_aliases", buildSimulationDriverAliasColumns("simulation_driver_aliases"), simulationDriverAliasScanMapper)
...
func simulationDriverAliasScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*SimulationDriverAlias, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *SimulationDriverAlias) any
	}
	targets := make([]target, 0, 8)
	for i, col := range cols {
		switch col {
		case "id":
			targets = append(targets, target{i, func(o *SimulationDriverAlias) any { return &o.ID }})
		case "driver_id":
			targets = append(targets, target{i, func(o *SimulationDriverAlias) any { return &o.DriverID }})
		case "simulation_id":
			targets = append(targets, target{i, func(o *SimulationDriverAlias) any { return &o.SimulationID }})
		case "simulation_driver_id":
			targets = append(targets, target{i, func(o *SimulationDriverAlias) any { return &o.SimulationDriverID }})
		case "created_at":
			targets = append(targets, target{i, func(o *SimulationDriverAlias) any { return &o.CreatedAt }})
		case "updated_at":
			targets = append(targets, target{i, func(o *SimulationDriverAlias) any { return &o.UpdatedAt }})
		case "created_by":
			targets = append(targets, target{i, func(o *SimulationDriverAlias) any { return &o.CreatedBy }})
		case "updated_by":
			targets = append(targets, target{i, func(o *SimulationDriverAlias) any { return &o.UpdatedBy }})
		}
	}

	return func(row *scan.Row) (any, error) {
			o := new(SimulationDriverAlias)
			for _, t := range targets {
				row.ScheduleScanByIndex(t.idx, t.dst(o))
			}
			return o, nil
		}, func(v any) (*SimulationDriverAlias, error) {
			return v.(*SimulationDriverAlias), nil
		}
}
...
func (o SimulationDriverAliasSlice) copyMatchingRows(from ...*SimulationDriverAlias) {
	fromByPK := make(map[int32]*SimulationDriverAlias, len(from))
...
type simulationDriverAliasWhere[Q psql.Filterable] struct {
	cols               simulationDriverAliasColumns
	...
	R                  simulationDriverAliasWhereR[Q]
}
...
func buildSimulationDriverAliasWhere[Q psql.Filterable](cols simulationDriverAliasColumns) simulationDriverAliasWhere[Q] {
	return simulationDriverAliasWhere[Q]{
		...
		R:                  simulationDriverAliasWhereR[Q]{cols: cols},
	}
}

// simulationDriverAliasWhereR holds the relationship-based filters of
// simulationDriverAliasWhere under the R namespace — mirroring the model's
// R struct — so they cannot collide with column-based filter fields.
type simulationDriverAliasWhereR[Q psql.Filterable] struct {
	cols simulationDriverAliasColumns
}
...
func (w simulationDriverAliasWhereR[Q]) HasDriver(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := psql.Select(
...
func (w simulationDriverAliasWhereR[Q]) HasSimulationRacingSim(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := psql.Select(
		sm.Columns(psql.Raw("1")),
