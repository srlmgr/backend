// Code generated by BobGen psql v0.50.0. DO NOT EDIT.
// This file is meant to be re-generated in place and/or deleted at any time.

package models

import (
	"context"
	"fmt"
	"io"
	"strings"
	"time"

	"github.com/aarondl/opt/null"
	"github.com/aarondl/opt/omit"
	"github.com/aarondl/opt/omitnull"
	"github.com/stephenafamo/bob"
	"github.com/stephenafamo/bob/dialect/psql"
	"github.com/stephenafamo/bob/dialect/psql/dialect"
	"github.com/stephenafamo/bob/dialect/psql/dm"
	"github.com/stephenafamo/bob/dialect/psql/sm"
	"github.com/stephenafamo/bob/dialect/psql/um"
	"github.com/stephenafamo/bob/expr"
	"github.com/stephenafamo/bob/mods"
	"github.com/stephenafamo/bob/orm"
	"github.com/stephenafamo/bob/types/pgtypes"
	"github.com/stephenafamo/scan"
)

// CarClass is an object representing the database table.
type CarClass struct {
	ID        int32     `db:"id,pk" `
	Name      string    `db:"name" `
	IsActive  bool      `db:"is_active" `
	CreatedAt time.Time `db:"created_at" `
	UpdatedAt time.Time `db:"updated_at" `
	CreatedBy string    `db:"created_by" `
	UpdatedBy string    `db:"updated_by" `

	R carClassR `db:"-" `

	C carClassC `db:"-" `
}

// CarClassSlice is an alias for a slice of pointers to CarClass.
// This should almost always be used instead of []*CarClass.
type CarClassSlice []*CarClass

// CarClasses contains methods to work with the car_classes table
var CarClasses = psql.NewTablex[*CarClass, CarClassSlice, *CarClassSetter]("", "car_classes", buildCarClassColumns("car_classes"), carClassScanMapper)

// CarClassesQuery is a query on the car_classes table
type CarClassesQuery = *psql.ViewQuery[*CarClass, CarClassSlice]

// carClassR is where relationships are stored.
type carClassR struct {
	BookingEntries        BookingEntrySlice         // booking_entries.booking_entries_car_class_id_fk
	CarClassesToCarModels CarClassesToCarModelSlice // car_classes_to_car_models.car_classes_to_car_models_car_class_id_fk
	ResultEntries         ResultEntrySlice          // result_entries.result_entries_car_class_id_fk
	SeasonCarClasses      SeasonCarClassSlice       // season_car_classes.season_car_classes_car_class_id_fk
	// Loaded reports whether each relationship has been loaded.
	// A relationship's bool is set by Load*, Preload, ThenLoad, factory builds,
	// and to-one Attach/Insert operations. To-many Attach/Insert operations leave it unchanged.
	Loaded carClassRLoaded `db:"-" `
}

// carClassRLoaded tracks which relationships on CarClass have been loaded.
type carClassRLoaded struct {
	BookingEntries        bool // booking_entries.booking_entries_car_class_id_fk
	CarClassesToCarModels bool // car_classes_to_car_models.car_classes_to_car_models_car_class_id_fk
	ResultEntries         bool // result_entries.result_entries_car_class_id_fk
	SeasonCarClasses      bool // season_car_classes.season_car_classes_car_class_id_fk
}

func buildCarClassColumns(tableName string) carClassColumns {
	columnsExpr := expr.NewColumnsExpr(
		"id", "name", "is_active", "created_at", "updated_at", "created_by", "updated_by",
	)

	if tableName != "" {
		columnsExpr = columnsExpr.WithParent(tableName)
	}

	return carClassColumns{
		ColumnsExpr: columnsExpr,
		tableAlias:  tableName,
		ID:          buildCarClassColumn(tableName, "id"),
		Name:        buildCarClassColumn(tableName, "name"),
		IsActive:    buildCarClassColumn(tableName, "is_active"),
		CreatedAt:   buildCarClassColumn(tableName, "created_at"),
		UpdatedAt:   buildCarClassColumn(tableName, "updated_at"),
		CreatedBy:   buildCarClassColumn(tableName, "created_by"),
		UpdatedBy:   buildCarClassColumn(tableName, "updated_by"),
	}
}

type carClassColumns struct {
	expr.ColumnsExpr
	tableAlias string
	ID         carClassColumn
	Name       carClassColumn
	IsActive   carClassColumn
	CreatedAt  carClassColumn
	UpdatedAt  carClassColumn
	CreatedBy  carClassColumn
	UpdatedBy  carClassColumn
}

// Alias returns the current table alias for the columns set.
func (c carClassColumns) Alias() string {
	return c.tableAlias
}

// AliasedAs returns a copy of the columns set qualified by tableName.
func (carClassColumns) AliasedAs(tableName string) carClassColumns {
	return buildCarClassColumns(tableName)
}

// Unqualified returns a copy of the columns set without table qualification.
func (c carClassColumns) Unqualified() carClassColumns {
	return buildCarClassColumns("")
}

func buildCarClassColumn(alias, name string) carClassColumn {
	return carClassColumn{
		Expression: psql.Quote(alias, name),
		alias:      alias,
		name:       name,
	}
}

type carClassColumn struct {
	psql.Expression
	alias string
	name  string
}

// Name returns the unqualified column name.
func (c carClassColumn) Name() string {
	return c.name
}

// ShouldOmitParens prevents automatic parenthesis wrapping in expression builders.
func (c carClassColumn) ShouldOmitParens() bool {
	return true
}

// CarClassSetter is used for insert/upsert/update operations
// All values are optional, and do not have to be set
// Generated columns are not included
type CarClassSetter struct {
	ID        omit.Val[int32]     `db:"id,pk" `
	Name      omit.Val[string]    `db:"name" `
	IsActive  omit.Val[bool]      `db:"is_active" `
	CreatedAt omit.Val[time.Time] `db:"created_at" `
	UpdatedAt omit.Val[time.Time] `db:"updated_at" `
	CreatedBy omit.Val[string]    `db:"created_by" `
	UpdatedBy omit.Val[string]    `db:"updated_by" `
}

func (s CarClassSetter) SetColumns() []string {
	vals := make([]string, 0, 7)
	if s.ID.IsValue() {
		vals = append(vals, "id")
	}
	if s.Name.IsValue() {
		vals = append(vals, "name")
	}
	if s.IsActive.IsValue() {
		vals = append(vals, "is_active")
	}
	if s.CreatedAt.IsValue() {
		vals = append(vals, "created_at")
	}
	if s.UpdatedAt.IsValue() {
		vals = append(vals, "updated_at")
	}
	if s.CreatedBy.IsValue() {
		vals = append(vals, "created_by")
	}
	if s.UpdatedBy.IsValue() {
		vals = append(vals, "updated_by")
	}
	return vals
}

func (s CarClassSetter) Overwrite(t *CarClass) {
	if s.ID.IsValue() {
		t.ID = s.ID.MustGet()
	}
	if s.Name.IsValue() {
		t.Name = s.Name.MustGet()
	}
	if s.IsActive.IsValue() {
		t.IsActive = s.IsActive.MustGet()
	}
	if s.CreatedAt.IsValue() {
		t.CreatedAt = s.CreatedAt.MustGet()
	}
	if s.UpdatedAt.IsValue() {
		t.UpdatedAt = s.UpdatedAt.MustGet()
	}
	if s.CreatedBy.IsValue() {
		t.CreatedBy = s.CreatedBy.MustGet()
	}
	if s.UpdatedBy.IsValue() {
		t.UpdatedBy = s.UpdatedBy.MustGet()
	}
}

func (s *CarClassSetter) Apply(q *dialect.InsertQuery) {
	q.AppendHooks(func(ctx context.Context, exec bob.Executor) (context.Context, error) {
		return CarClasses.BeforeInsertHooks.RunHooks(ctx, exec, s)
	})

	q.AppendValues(
		bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
			if s.ID.IsUnset() {
				return psql.Raw("DEFAULT").WriteSQL(ctx, w, d, start)
			}
			return psql.Arg(s.ID.MustGet()).WriteSQL(ctx, w, d, start)
		}), bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
			if s.Name.IsUnset() {
				return psql.Raw("DEFAULT").WriteSQL(ctx, w, d, start)
			}
			return psql.Arg(s.Name.MustGet()).WriteSQL(ctx, w, d, start)
		}), bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
			if s.IsActive.IsUnset() {
				return psql.Raw("DEFAULT").WriteSQL(ctx, w, d, start)
			}
			return psql.Arg(s.IsActive.MustGet()).WriteSQL(ctx, w, d, start)
		}), bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
			if s.CreatedAt.IsUnset() {
				return psql.Raw("DEFAULT").WriteSQL(ctx, w, d, start)
			}
			return psql.Arg(s.CreatedAt.MustGet()).WriteSQL(ctx, w, d, start)
		}), bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
			if s.UpdatedAt.IsUnset() {
				return psql.Raw("DEFAULT").WriteSQL(ctx, w, d, start)
			}
			return psql.Arg(s.UpdatedAt.MustGet()).WriteSQL(ctx, w, d, start)
		}), bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
			if s.CreatedBy.IsUnset() {
				return psql.Raw("DEFAULT").WriteSQL(ctx, w, d, start)
			}
			return psql.Arg(s.CreatedBy.MustGet()).WriteSQL(ctx, w, d, start)
		}), bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
			if s.UpdatedBy.IsUnset() {
				return psql.Raw("DEFAULT").WriteSQL(ctx, w, d, start)
			}
			return psql.Arg(s.UpdatedBy.MustGet()).WriteSQL(ctx, w, d, start)
		}))
}

func (s CarClassSetter) UpdateMod() bob.Mod[*dialect.UpdateQuery] {
	return um.Set(s.Expressions()...)
}

func (s CarClassSetter) Expressions(prefix ...string) []bob.Expression {
	exprs := make([]bob.Expression, 0, 7)

	if s.ID.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			psql.Quote(append(prefix, "id")...),
			psql.Arg(s.ID),
		}})
	}

	if s.Name.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			psql.Quote(append(prefix, "name")...),
			psql.Arg(s.Name),
		}})
	}

	if s.IsActive.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			psql.Quote(append(prefix, "is_active")...),
			psql.Arg(s.IsActive),
		}})
	}

	if s.CreatedAt.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			psql.Quote(append(prefix, "created_at")...),
			psql.Arg(s.CreatedAt),
		}})
	}

	if s.UpdatedAt.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			psql.Quote(append(prefix, "updated_at")...),
			psql.Arg(s.UpdatedAt),
		}})
	}

	if s.CreatedBy.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			psql.Quote(append(prefix, "created_by")...),
			psql.Arg(s.CreatedBy),
		}})
	}

	if s.UpdatedBy.IsValue() {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			psql.Quote(append(prefix, "updated_by")...),
			psql.Arg(s.UpdatedBy),
		}})
	}

	return exprs
}

// scan.Mapper[*T] without reflection: maps columns to fields by index.
func carClassScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*CarClass, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *CarClass) any
	}
	targets := make([]target, 0, 7)
	for i, col := range cols {
		switch col {
		case "id":
			targets = append(targets, target{i, func(o *CarClass) any { return &o.ID }})
		case "name":
			targets = append(targets, target{i, func(o *CarClass) any { return &o.Name }})
		case "is_active":
			targets = append(targets, target{i, func(o *CarClass) any { return &o.IsActive }})
		case "created_at":
			targets = append(targets, target{i, func(o *CarClass) any { return &o.CreatedAt }})
		case "updated_at":
			targets = append(targets, target{i, func(o *CarClass) any { return &o.UpdatedAt }})
		case "created_by":
			targets = append(targets, target{i, func(o *CarClass) any { return &o.CreatedBy }})
		case "updated_by":
			targets = append(targets, target{i, func(o *CarClass) any { return &o.UpdatedBy }})
		}
	}

	return func(row *scan.Row) (any, error) {
			o := new(CarClass)
			for _, t := range targets {
				row.ScheduleScanByIndex(t.idx, t.dst(o))
			}
			return o, nil
		}, func(v any) (*CarClass, error) {
			return v.(*CarClass), nil
		}
}

// FindCarClass retrieves a single record by primary key
// If cols is empty Find will return all columns.
func FindCarClass(ctx context.Context, exec bob.Executor, IDPK int32, cols ...string) (*CarClass, error) {
	if len(cols) == 0 {
		return CarClasses.Query(
			sm.Where(CarClasses.Columns.ID.EQ(psql.Arg(IDPK))),
		).One(ctx, exec)
	}

	return CarClasses.Query(
		sm.Where(CarClasses.Columns.ID.EQ(psql.Arg(IDPK))),
		sm.Columns(CarClasses.Columns.Only(cols...)),
	).One(ctx, exec)
}

// CarClassExists checks the presence of a single record by primary key
func CarClassExists(ctx context.Context, exec bob.Executor, IDPK int32) (bool, error) {
	return CarClasses.Query(
		sm.Where(CarClasses.Columns.ID.EQ(psql.Arg(IDPK))),
	).Exists(ctx, exec)
}

// AfterQueryHook is called after CarClass is retrieved from the database
func (o *CarClass) AfterQueryHook(ctx context.Context, exec bob.Executor, queryType bob.QueryType) error {
	var err error

	switch queryType {
	case bob.QueryTypeSelect:
		ctx, err = CarClasses.AfterSelectHooks.RunHooks(ctx, exec, CarClassSlice{o})
	case bob.QueryTypeInsert:
		ctx, err = CarClasses.AfterInsertHooks.RunHooks(ctx, exec, CarClassSlice{o})
	case bob.QueryTypeUpdate:
		ctx, err = CarClasses.AfterUpdateHooks.RunHooks(ctx, exec, CarClassSlice{o})
	case bob.QueryTypeDelete:
		ctx, err = CarClasses.AfterDeleteHooks.RunHooks(ctx, exec, CarClassSlice{o})
	case bob.QueryTypeMerge:
		ctx, err = CarClasses.AfterMergeHooks.RunHooks(ctx, exec, CarClassSlice{o})
	}

	return err
}

// primaryKeyVals returns the primary key values of the CarClass
func (o *CarClass) primaryKeyVals() bob.Expression {
	return psql.Arg(o.ID)
}

func (o *CarClass) pkEQ() dialect.Expression {
	return psql.Quote("car_classes", "id").EQ(bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
		return o.primaryKeyVals().WriteSQL(ctx, w, d, start)
	}))
}

// Update uses an executor to update the CarClass
func (o *CarClass) Update(ctx context.Context, exec bob.Executor, s *CarClassSetter) error {
	v, err := CarClasses.Update(s.UpdateMod(), um.Where(o.pkEQ())).One(ctx, exec)
	if err != nil {
		return err
	}

	o.R = v.R
	*o = *v

	return nil
}

// Delete deletes a single CarClass record with an executor
func (o *CarClass) Delete(ctx context.Context, exec bob.Executor) error {
	_, err := CarClasses.Delete(dm.Where(o.pkEQ())).Exec(ctx, exec)
	return err
}

// Reload refreshes the CarClass using the executor
func (o *CarClass) Reload(ctx context.Context, exec bob.Executor) error {
	o2, err := CarClasses.Query(
		sm.Where(CarClasses.Columns.ID.EQ(psql.Arg(o.ID))),
	).One(ctx, exec)
	if err != nil {
		return err
	}
	o2.R = o.R
	*o = *o2

	return nil
}

// AfterQueryHook is called after CarClassSlice is retrieved from the database
func (o CarClassSlice) AfterQueryHook(ctx context.Context, exec bob.Executor, queryType bob.QueryType) error {
	var err error

	switch queryType {
	case bob.QueryTypeSelect:
		ctx, err = CarClasses.AfterSelectHooks.RunHooks(ctx, exec, o)
	case bob.QueryTypeInsert:
		ctx, err = CarClasses.AfterInsertHooks.RunHooks(ctx, exec, o)
	case bob.QueryTypeUpdate:
		ctx, err = CarClasses.AfterUpdateHooks.RunHooks(ctx, exec, o)
	case bob.QueryTypeDelete:
		ctx, err = CarClasses.AfterDeleteHooks.RunHooks(ctx, exec, o)
	case bob.QueryTypeMerge:
		ctx, err = CarClasses.AfterMergeHooks.RunHooks(ctx, exec, o)
	}

	return err
}

func (o CarClassSlice) pkIN() dialect.Expression {
	if len(o) == 0 {
		return psql.Raw("NULL")
	}

	return psql.Quote("car_classes", "id").In(bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
		pkPairs := make([]bob.Expression, len(o))
		for i, row := range o {
			pkPairs[i] = row.primaryKeyVals()
		}
		return bob.ExpressSlice(ctx, w, d, start, pkPairs, "", ", ", "")
	}))
}

// copyMatchingRows finds models in the given slice that have the same primary key
// then it first copies the existing relationships from the old model to the new model
// and then replaces the old model in the slice with the new model
func (o CarClassSlice) copyMatchingRows(from ...*CarClass) {
	fromByPK := make(map[int32]*CarClass, len(from))
	for _, new := range from {
		// keep the first row for each key, like the nested loop did
		if _, ok := fromByPK[new.ID]; !ok {
			fromByPK[new.ID] = new
		}
	}

	for i, old := range o {
		new, ok := fromByPK[old.ID]
		if !ok {
			continue
		}
		new.R = old.R
		o[i] = new
	}
}

// UpdateMod modifies an update query with "WHERE primary_key IN (o...)"
func (o CarClassSlice) UpdateMod() bob.Mod[*dialect.UpdateQuery] {
	return bob.ModFunc[*dialect.UpdateQuery](func(q *dialect.UpdateQuery) {
		q.AppendHooks(func(ctx context.Context, exec bob.Executor) (context.Context, error) {
			return CarClasses.BeforeUpdateHooks.RunHooks(ctx, exec, o)
		})

		q.AppendLoader(bob.LoaderFunc(func(ctx context.Context, exec bob.Executor, retrieved any) error {
			var err error
			switch retrieved := retrieved.(type) {
			case *CarClass:
				o.copyMatchingRows(retrieved)
			case []*CarClass:
				o.copyMatchingRows(retrieved...)
			case CarClassSlice:
				o.copyMatchingRows(retrieved...)
			default:
				// If the retrieved value is not a CarClass or a slice of CarClass
				// then run the AfterUpdateHooks on the slice
				_, err = CarClasses.AfterUpdateHooks.RunHooks(ctx, exec, o)
			}

			return err
		}))

		q.AppendWhere(o.pkIN())
	})
}

// DeleteMod modifies an delete query with "WHERE primary_key IN (o...)"
func (o CarClassSlice) DeleteMod() bob.Mod[*dialect.DeleteQuery] {
	return bob.ModFunc[*dialect.DeleteQuery](func(q *dialect.DeleteQuery) {
		q.AppendHooks(func(ctx context.Context, exec bob.Executor) (context.Context, error) {
			return CarClasses.BeforeDeleteHooks.RunHooks(ctx, exec, o)
		})

		q.AppendLoader(bob.LoaderFunc(func(ctx context.Context, exec bob.Executor, retrieved any) error {
			var err error
			switch retrieved := retrieved.(type) {
			case *CarClass:
				o.copyMatchingRows(retrieved)
			case []*CarClass:
				o.copyMatchingRows(retrieved...)
			case CarClassSlice:
				o.copyMatchingRows(retrieved...)
			default:
				// If the retrieved value is not a CarClass or a slice of CarClass
				// then run the AfterDeleteHooks on the slice
				_, err = CarClasses.AfterDeleteHooks.RunHooks(ctx, exec, o)
			}

			return err
		}))

		q.AppendWhere(o.pkIN())
	})
}

// MergeMod modifies a merge query to run BeforeMergeHooks and AfterMergeHooks
// and updates the slice with the returned rows.
func (o CarClassSlice) MergeMod() bob.Mod[*dialect.MergeQuery] {
	return bob.ModFunc[*dialect.MergeQuery](func(q *dialect.MergeQuery) {
		q.AppendHooks(func(ctx context.Context, exec bob.Executor) (context.Context, error) {
			return CarClasses.BeforeMergeHooks.RunHooks(ctx, exec, o)
		})

		q.AppendLoader(bob.LoaderFunc(func(ctx context.Context, exec bob.Executor, retrieved any) error {
			var err error
			switch retrieved := retrieved.(type) {
			case *CarClass:
				o.copyMatchingRows(retrieved)
			case []*CarClass:
				o.copyMatchingRows(retrieved...)
			case CarClassSlice:
				o.copyMatchingRows(retrieved...)
			default:
				// If the retrieved value is not a CarClass or a slice of CarClass
				// then run the AfterMergeHooks on the slice
				_, err = CarClasses.AfterMergeHooks.RunHooks(ctx, exec, o)
			}

			return err
		}))
	})
}

func (o CarClassSlice) UpdateAll(ctx context.Context, exec bob.Executor, vals CarClassSetter) error {
	if len(o) == 0 {
		return nil
	}

	_, err := CarClasses.Update(vals.UpdateMod(), o.UpdateMod()).All(ctx, exec)
	return err
}

func (o CarClassSlice) DeleteAll(ctx context.Context, exec bob.Executor) error {
	if len(o) == 0 {
		return nil
	}

	_, err := CarClasses.Delete(o.DeleteMod()).Exec(ctx, exec)
	return err
}

func (o CarClassSlice) ReloadAll(ctx context.Context, exec bob.Executor) error {
	if len(o) == 0 {
		return nil
	}

	o2, err := CarClasses.Query(sm.Where(o.pkIN())).All(ctx, exec)
	if err != nil {
		return err
	}

	o.copyMatchingRows(o2...)

	return nil
}

// BookingEntries starts a query for related objects on booking_entries
func (o *CarClass) BookingEntries(mods ...bob.Mod[*dialect.SelectQuery]) BookingEntriesQuery {
	return BookingEntries.Query(append(mods,
		sm.Where(BookingEntries.Columns.CarClassID.EQ(psql.Arg(o.ID))),
	)...)
}

func (os CarClassSlice) BookingEntries(mods ...bob.Mod[*dialect.SelectQuery]) BookingEntriesQuery {
	pkID := make(pgtypes.Array[int32], 0, len(os))

	for _, o := range os {
		if o == nil {
			continue
		}
		pkID = append(pkID, o.ID)
	}
	PKArgExpr := psql.Any(psql.Cast(psql.Arg(pkID), "integer[]"))

	return BookingEntries.Query(append(mods,
		sm.Where(BookingEntries.Columns.CarClassID.EQ(PKArgExpr)),
	)...)
}

// CarClassesToCarModels starts a query for related objects on car_classes_to_car_models
func (o *CarClass) CarClassesToCarModels(mods ...bob.Mod[*dialect.SelectQuery]) CarClassesToCarModelsQuery {
	return CarClassesToCarModels.Query(append(mods,
		sm.Where(CarClassesToCarModels.Columns.CarClassID.EQ(psql.Arg(o.ID))),
	)...)
}

func (os CarClassSlice) CarClassesToCarModels(mods ...bob.Mod[*dialect.SelectQuery]) CarClassesToCarModelsQuery {
	pkID := make(pgtypes.Array[int32], 0, len(os))

	for _, o := range os {
		if o == nil {
			continue
		}
		pkID = append(pkID, o.ID)
	}
	PKArgExpr := psql.Any(psql.Cast(psql.Arg(pkID), "integer[]"))

	return CarClassesToCarModels.Query(append(mods,
		sm.Where(CarClassesToCarModels.Columns.CarClassID.EQ(PKArgExpr)),
	)...)
}

// ResultEntries starts a query for related objects on result_entries
func (o *CarClass) ResultEntries(mods ...bob.Mod[*dialect.SelectQuery]) ResultEntriesQuery {
	return ResultEntries.Query(append(mods,
		sm.Where(ResultEntries.Columns.CarClassID.EQ(psql.Arg(o.ID))),
	)...)
}

func (os CarClassSlice) ResultEntries(mods ...bob.Mod[*dialect.SelectQuery]) ResultEntriesQuery {
	pkID := make(pgtypes.Array[int32], 0, len(os))

	for _, o := range os {
		if o == nil {
			continue
		}
		pkID = append(pkID, o.ID)
	}
	PKArgExpr := psql.Any(psql.Cast(psql.Arg(pkID), "integer[]"))

	return ResultEntries.Query(append(mods,
		sm.Where(ResultEntries.Columns.CarClassID.EQ(PKArgExpr)),
	)...)
}

// SeasonCarClasses starts a query for related objects on season_car_classes
func (o *CarClass) SeasonCarClasses(mods ...bob.Mod[*dialect.SelectQuery]) SeasonCarClassesQuery {
	return SeasonCarClasses.Query(append(mods,
		sm.Where(SeasonCarClasses.Columns.CarClassID.EQ(psql.Arg(o.ID))),
	)...)
}

func (os CarClassSlice) SeasonCarClasses(mods ...bob.Mod[*dialect.SelectQuery]) SeasonCarClassesQuery {
	pkID := make(pgtypes.Array[int32], 0, len(os))

	for _, o := range os {
		if o == nil {
			continue
		}
		pkID = append(pkID, o.ID)
	}
	PKArgExpr := psql.Any(psql.Cast(psql.Arg(pkID), "integer[]"))

	return SeasonCarClasses.Query(append(mods,
		sm.Where(SeasonCarClasses.Columns.CarClassID.EQ(PKArgExpr)),
	)...)
}

func insertCarClassBookingEntries0(ctx context.Context, exec bob.Executor, bookingEntries1 []*BookingEntrySetter, carClass0 *CarClass) (BookingEntrySlice, error) {
	for i := range bookingEntries1 {
		bookingEntries1[i].CarClassID = omitnull.From(carClass0.ID)
	}

	ret, err := BookingEntries.Insert(bob.ToMods(bookingEntries1...)).All(ctx, exec)
	if err != nil {
		return ret, fmt.Errorf("insertCarClassBookingEntries0: %w", err)
	}

	return ret, nil
}

func attachCarClassBookingEntries0(ctx context.Context, exec bob.Executor, count int, bookingEntries1 BookingEntrySlice, carClass0 *CarClass) (BookingEntrySlice, error) {
	setter := &BookingEntrySetter{
		CarClassID: omitnull.From(carClass0.ID),
	}

	err := bookingEntries1.UpdateAll(ctx, exec, *setter)
	if err != nil {
		return nil, fmt.Errorf("attachCarClassBookingEntries0: %w", err)
	}

	return bookingEntries1, nil
}

func (carClass0 *CarClass) InsertBookingEntries(ctx context.Context, exec bob.Executor, related ...*BookingEntrySetter) error {
	if len(related) == 0 {
		return nil
	}

	var err error

	bookingEntries1, err := insertCarClassBookingEntries0(ctx, exec, related, carClass0)
	if err != nil {
		return err
	}

	carClass0.R.BookingEntries = append(carClass0.R.BookingEntries, bookingEntries1...)

	for _, rel := range bookingEntries1 {
		rel.R.CarClass = carClass0
		rel.R.Loaded.CarClass = true
	}
	return nil
}

func (carClass0 *CarClass) AttachBookingEntries(ctx context.Context, exec bob.Executor, related ...*BookingEntry) error {
	if len(related) == 0 {
		return nil
	}

	var err error
	bookingEntries1 := BookingEntrySlice(related)

	_, err = attachCarClassBookingEntries0(ctx, exec, len(related), bookingEntries1, carClass0)
	if err != nil {
		return err
	}

	carClass0.R.BookingEntries = append(carClass0.R.BookingEntries, bookingEntries1...)

	for _, rel := range related {
		rel.R.CarClass = carClass0
		rel.R.Loaded.CarClass = true
	}

	return nil
}

func insertCarClassCarClassesToCarModels0(ctx context.Context, exec bob.Executor, carClassesToCarModels1 []*CarClassesToCarModelSetter, carClass0 *CarClass) (CarClassesToCarModelSlice, error) {
	for i := range carClassesToCarModels1 {
		carClassesToCarModels1[i].CarClassID = omit.From(carClass0.ID)
	}

	ret, err := CarClassesToCarModels.Insert(bob.ToMods(carClassesToCarModels1...)).All(ctx, exec)
	if err != nil {
		return ret, fmt.Errorf("insertCarClassCarClassesToCarModels0: %w", err)
	}

	return ret, nil
}

func attachCarClassCarClassesToCarModels0(ctx context.Context, exec bob.Executor, count int, carClassesToCarModels1 CarClassesToCarModelSlice, carClass0 *CarClass) (CarClassesToCarModelSlice, error) {
	setter := &CarClassesToCarModelSetter{
		CarClassID: omit.From(carClass0.ID),
	}

	err := carClassesToCarModels1.UpdateAll(ctx, exec, *setter)
	if err != nil {
		return nil, fmt.Errorf("attachCarClassCarClassesToCarModels0: %w", err)
	}

	return carClassesToCarModels1, nil
}

func (carClass0 *CarClass) InsertCarClassesToCarModels(ctx context.Context, exec bob.Executor, related ...*CarClassesToCarModelSetter) error {
	if len(related) == 0 {
		return nil
	}

	var err error

	carClassesToCarModels1, err := insertCarClassCarClassesToCarModels0(ctx, exec, related, carClass0)
	if err != nil {
		return err
	}

	carClass0.R.CarClassesToCarModels = append(carClass0.R.CarClassesToCarModels, carClassesToCarModels1...)

	for _, rel := range carClassesToCarModels1 {
		rel.R.CarClass = carClass0
		rel.R.Loaded.CarClass = true
	}
	return nil
}

func (carClass0 *CarClass) AttachCarClassesToCarModels(ctx context.Context, exec bob.Executor, related ...*CarClassesToCarModel) error {
	if len(related) == 0 {
		return nil
	}

	var err error
	carClassesToCarModels1 := CarClassesToCarModelSlice(related)

	_, err = attachCarClassCarClassesToCarModels0(ctx, exec, len(related), carClassesToCarModels1, carClass0)
	if err != nil {
		return err
	}

	carClass0.R.CarClassesToCarModels = append(carClass0.R.CarClassesToCarModels, carClassesToCarModels1...)

	for _, rel := range related {
		rel.R.CarClass = carClass0
		rel.R.Loaded.CarClass = true
	}

	return nil
}

func insertCarClassResultEntries0(ctx context.Context, exec bob.Executor, resultEntries1 []*ResultEntrySetter, carClass0 *CarClass) (ResultEntrySlice, error) {
	for i := range resultEntries1 {
		resultEntries1[i].CarClassID = omitnull.From(carClass0.ID)
	}

	ret, err := ResultEntries.Insert(bob.ToMods(resultEntries1...)).All(ctx, exec)
	if err != nil {
		return ret, fmt.Errorf("insertCarClassResultEntries0: %w", err)
	}

	return ret, nil
}

func attachCarClassResultEntries0(ctx context.Context, exec bob.Executor, count int, resultEntries1 ResultEntrySlice, carClass0 *CarClass) (ResultEntrySlice, error) {
	setter := &ResultEntrySetter{
		CarClassID: omitnull.From(carClass0.ID),
	}

	err := resultEntries1.UpdateAll(ctx, exec, *setter)
	if err != nil {
		return nil, fmt.Errorf("attachCarClassResultEntries0: %w", err)
	}

	return resultEntries1, nil
}

func (carClass0 *CarClass) InsertResultEntries(ctx context.Context, exec bob.Executor, related ...*ResultEntrySetter) error {
	if len(related) == 0 {
		return nil
	}

	var err error

	resultEntries1, err := insertCarClassResultEntries0(ctx, exec, related, carClass0)
	if err != nil {
		return err
	}

	carClass0.R.ResultEntries = append(carClass0.R.ResultEntries, resultEntries1...)

	for _, rel := range resultEntries1 {
		rel.R.CarClass = carClass0
		rel.R.Loaded.CarClass = true
	}
	return nil
}

func (carClass0 *CarClass) AttachResultEntries(ctx context.Context, exec bob.Executor, related ...*ResultEntry) error {
	if len(related) == 0 {
		return nil
	}

	var err error
	resultEntries1 := ResultEntrySlice(related)

	_, err = attachCarClassResultEntries0(ctx, exec, len(related), resultEntries1, carClass0)
	if err != nil {
		return err
	}

	carClass0.R.ResultEntries = append(carClass0.R.ResultEntries, resultEntries1...)

	for _, rel := range related {
		rel.R.CarClass = carClass0
		rel.R.Loaded.CarClass = true
	}

	return nil
}

func insertCarClassSeasonCarClasses0(ctx context.Context, exec bob.Executor, seasonCarClasses1 []*SeasonCarClassSetter, carClass0 *CarClass) (SeasonCarClassSlice, error) {
	for i := range seasonCarClasses1 {
		seasonCarClasses1[i].CarClassID = omit.From(carClass0.ID)
	}

	ret, err := SeasonCarClasses.Insert(bob.ToMods(seasonCarClasses1...)).All(ctx, exec)
	if err != nil {
		return ret, fmt.Errorf("insertCarClassSeasonCarClasses0: %w", err)
	}

	return ret, nil
}

func attachCarClassSeasonCarClasses0(ctx context.Context, exec bob.Executor, count int, seasonCarClasses1 SeasonCarClassSlice, carClass0 *CarClass) (SeasonCarClassSlice, error) {
	setter := &SeasonCarClassSetter{
		CarClassID: omit.From(carClass0.ID),
	}

	err := seasonCarClasses1.UpdateAll(ctx, exec, *setter)
	if err != nil {
		return nil, fmt.Errorf("attachCarClassSeasonCarClasses0: %w", err)
	}

	return seasonCarClasses1, nil
}

func (carClass0 *CarClass) InsertSeasonCarClasses(ctx context.Context, exec bob.Executor, related ...*SeasonCarClassSetter) error {
	if len(related) == 0 {
		return nil
	}

	var err error

	seasonCarClasses1, err := insertCarClassSeasonCarClasses0(ctx, exec, related, carClass0)
	if err != nil {
		return err
	}

	carClass0.R.SeasonCarClasses = append(carClass0.R.SeasonCarClasses, seasonCarClasses1...)

	for _, rel := range seasonCarClasses1 {
		rel.R.CarClass = carClass0
		rel.R.Loaded.CarClass = true
	}
	return nil
}

func (carClass0 *CarClass) AttachSeasonCarClasses(ctx context.Context, exec bob.Executor, related ...*SeasonCarClass) error {
	if len(related) == 0 {
		return nil
	}

	var err error
	seasonCarClasses1 := SeasonCarClassSlice(related)

	_, err = attachCarClassSeasonCarClasses0(ctx, exec, len(related), seasonCarClasses1, carClass0)
	if err != nil {
		return err
	}

	carClass0.R.SeasonCarClasses = append(carClass0.R.SeasonCarClasses, seasonCarClasses1...)

	for _, rel := range related {
		rel.R.CarClass = carClass0
		rel.R.Loaded.CarClass = true
	}

	return nil
}

type carClassWhere[Q psql.Filterable] struct {
	cols      carClassColumns
	ID        psql.WhereMod[Q, int32]
	Name      psql.WhereMod[Q, string]
	IsActive  psql.WhereMod[Q, bool]
	CreatedAt psql.WhereMod[Q, time.Time]
	UpdatedAt psql.WhereMod[Q, time.Time]
	CreatedBy psql.WhereMod[Q, string]
	UpdatedBy psql.WhereMod[Q, string]
	R         carClassWhereR[Q]
}

func (carClassWhere[Q]) AliasedAs(alias string) carClassWhere[Q] {
	return buildCarClassWhere[Q](buildCarClassColumns(alias))
}

func buildCarClassWhere[Q psql.Filterable](cols carClassColumns) carClassWhere[Q] {
	return carClassWhere[Q]{
		cols:      cols,
		ID:        psql.Where[Q, int32](cols.ID.Expression),
		Name:      psql.Where[Q, string](cols.Name.Expression),
		IsActive:  psql.Where[Q, bool](cols.IsActive.Expression),
		CreatedAt: psql.Where[Q, time.Time](cols.CreatedAt.Expression),
		UpdatedAt: psql.Where[Q, time.Time](cols.UpdatedAt.Expression),
		CreatedBy: psql.Where[Q, string](cols.CreatedBy.Expression),
		UpdatedBy: psql.Where[Q, string](cols.UpdatedBy.Expression),
		R:         carClassWhereR[Q]{cols: cols},
	}
}

// carClassWhereR holds the relationship-based filters of
// carClassWhere under the R namespace — mirroring the model's
// R struct — so they cannot collide with column-based filter fields.
type carClassWhereR[Q psql.Filterable] struct {
	cols carClassColumns
}

// HasBookingEntries filters parents that have a matching BookingEntries using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to BookingEntries).
func (w carClassWhereR[Q]) HasBookingEntries(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := psql.Select(
		sm.Columns(psql.Raw("1")),
		sm.From(BookingEntries.NameExpr()),
		sm.Where(BookingEntries.Columns.CarClassID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: psql.Exists(q)}
}

// HasCarClassesToCarModels filters parents that have a matching CarClassesToCarModels using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to CarClassesToCarModels).
func (w carClassWhereR[Q]) HasCarClassesToCarModels(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := psql.Select(
		sm.Columns(psql.Raw("1")),
		sm.From(CarClassesToCarModels.NameExpr()),
		sm.Where(CarClassesToCarModels.Columns.CarClassID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: psql.Exists(q)}
}

// HasResultEntries filters parents that have a matching ResultEntries using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to ResultEntries).
func (w carClassWhereR[Q]) HasResultEntries(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := psql.Select(
		sm.Columns(psql.Raw("1")),
		sm.From(ResultEntries.NameExpr()),
		sm.Where(ResultEntries.Columns.CarClassID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: psql.Exists(q)}
}

// HasSeasonCarClasses filters parents that have a matching SeasonCarClasses using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to SeasonCarClasses).
func (w carClassWhereR[Q]) HasSeasonCarClasses(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := psql.Select(
		sm.Columns(psql.Raw("1")),
		sm.From(SeasonCarClasses.NameExpr()),
		sm.Where(SeasonCarClasses.Columns.CarClassID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: psql.Exists(q)}
}

// NULL-tolerant scan buffer for carClassScanMapperNullable:
// on a LEFT JOIN miss every column comes back NULL, so each field uses the
// nullable version of the column type even when the column itself is NOT NULL.
type carClassPreloadBuf struct {
	ID        null.Val[int32]
	Name      null.Val[string]
	IsActive  null.Val[bool]
	CreatedAt null.Val[time.Time]
	UpdatedAt null.Val[time.Time]
	CreatedBy null.Val[string]
	UpdatedBy null.Val[string]
}

// carClassScanMapperNullable maps the preloaded carClass
// columns (prefixed with the runtime join alias) without reflection, while
// keeping the LEFT JOIN semantics of the reflection-based mapper: a row whose
// prefixed columns are all NULL yields nil (no child), and NULL values never
// error, they just leave the zero value in the field.
func carClassScanMapperNullable(prefix string) scan.Mapper[*CarClass] {
	return func(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*CarClass, error)) {
		// resolve the column names once per query, not per row
		type target struct {
			idx int
			dst func(b *carClassPreloadBuf) any
		}
		targets := make([]target, 0, 7)
		for i, col := range cols {
			name, ok := strings.CutPrefix(col, prefix)
			if !ok {
				continue
			}
			switch name {
			case "id":
				targets = append(targets, target{i, func(b *carClassPreloadBuf) any { return &b.ID }})
			case "name":
				targets = append(targets, target{i, func(b *carClassPreloadBuf) any { return &b.Name }})
			case "is_active":
				targets = append(targets, target{i, func(b *carClassPreloadBuf) any { return &b.IsActive }})
			case "created_at":
				targets = append(targets, target{i, func(b *carClassPreloadBuf) any { return &b.CreatedAt }})
			case "updated_at":
				targets = append(targets, target{i, func(b *carClassPreloadBuf) any { return &b.UpdatedAt }})
			case "created_by":
				targets = append(targets, target{i, func(b *carClassPreloadBuf) any { return &b.CreatedBy }})
			case "updated_by":
				targets = append(targets, target{i, func(b *carClassPreloadBuf) any { return &b.UpdatedBy }})
			}
		}

		// One scratch buffer reused for every row: before -> scan -> after runs
		// synchronously per row, the scheduled columns are fully overwritten each
		// row, and unselected columns stay zero, so a single buffer preserves the
		// per-row NULL semantics without allocating a buffer per parent row. The
		// returned child (o below) is always freshly allocated, so buf never
		// escapes past the after func.
		buf := new(carClassPreloadBuf)
		return func(row *scan.Row) (any, error) {
				for _, t := range targets {
					row.ScheduleScanByIndex(t.idx, t.dst(buf))
				}
				return buf, nil
			}, func(link any) (*CarClass, error) {
				buf := link.(*carClassPreloadBuf)

				// Same rule as the reflection mapper's row validator: the child
				// exists only if at least one of its columns is not NULL. Columns
				// not selected by the query are never scanned and stay invalid,
				// so this check also matches when only a subset is selected.
				if !(buf.ID.IsValue()) &&
					!(buf.Name.IsValue()) &&
					!(buf.IsActive.IsValue()) &&
					!(buf.CreatedAt.IsValue()) &&
					!(buf.UpdatedAt.IsValue()) &&
					!(buf.CreatedBy.IsValue()) &&
					!(buf.UpdatedBy.IsValue()) {
					return nil, nil
				}

				o := new(CarClass)
				if buf.ID.IsValue() {
					o.ID = buf.ID.MustGet()
				}
				if buf.Name.IsValue() {
					o.Name = buf.Name.MustGet()
				}
				if buf.IsActive.IsValue() {
					o.IsActive = buf.IsActive.MustGet()
				}
				if buf.CreatedAt.IsValue() {
					o.CreatedAt = buf.CreatedAt.MustGet()
				}
				if buf.UpdatedAt.IsValue() {
					o.UpdatedAt = buf.UpdatedAt.MustGet()
				}
				if buf.CreatedBy.IsValue() {
					o.CreatedBy = buf.CreatedBy.MustGet()
				}
				if buf.UpdatedBy.IsValue() {
					o.UpdatedBy = buf.UpdatedBy.MustGet()
				}
				return o, nil
			}
	}
}

func (o *CarClass) Preload(name string, retrieved any) error {
	if o == nil {
		return nil
	}

	switch name {
	case "BookingEntries":
		rels, ok := retrieved.(BookingEntrySlice)
		if !ok {
			return fmt.Errorf("carClass cannot load %T as %q", retrieved, name)
		}

		o.R.BookingEntries = rels
		o.R.Loaded.BookingEntries = true

		for _, rel := range rels {
			if rel != nil {
				rel.R.CarClass = o
				rel.R.Loaded.CarClass = true
			}
		}
		return nil
	case "CarClassesToCarModels":
		rels, ok := retrieved.(CarClassesToCarModelSlice)
		if !ok {
			return fmt.Errorf("carClass cannot load %T as %q", retrieved, name)
		}

		o.R.CarClassesToCarModels = rels
		o.R.Loaded.CarClassesToCarModels = true

		for _, rel := range rels {
			if rel != nil {
				rel.R.CarClass = o
				rel.R.Loaded.CarClass = true
			}
		}
		return nil
	case "ResultEntries":
		rels, ok := retrieved.(ResultEntrySlice)
		if !ok {
			return fmt.Errorf("carClass cannot load %T as %q", retrieved, name)
		}

		o.R.ResultEntries = rels
		o.R.Loaded.ResultEntries = true

		for _, rel := range rels {
			if rel != nil {
				rel.R.CarClass = o
				rel.R.Loaded.CarClass = true
			}
		}
		return nil
	case "SeasonCarClasses":
		rels, ok := retrieved.(SeasonCarClassSlice)
		if !ok {
			return fmt.Errorf("carClass cannot load %T as %q", retrieved, name)
		}

		o.R.SeasonCarClasses = rels
		o.R.Loaded.SeasonCarClasses = true

		for _, rel := range rels {
			if rel != nil {
				rel.R.CarClass = o
				rel.R.Loaded.CarClass = true
			}
		}
		return nil
	default:
		return fmt.Errorf("carClass has no relationship %q", name)
	}
}

type carClassPreloader struct{}

func buildCarClassPreloader() carClassPreloader {
	return carClassPreloader{}
}

type carClassThenLoader[Q orm.Loadable] struct {
	BookingEntries        func(...bob.Mod[*dialect.SelectQuery]) orm.Loader[Q]
	CarClassesToCarModels func(...bob.Mod[*dialect.SelectQuery]) orm.Loader[Q]
	ResultEntries         func(...bob.Mod[*dialect.SelectQuery]) orm.Loader[Q]
	SeasonCarClasses      func(...bob.Mod[*dialect.SelectQuery]) orm.Loader[Q]
}

func buildCarClassThenLoader[Q orm.Loadable]() carClassThenLoader[Q] {
	type BookingEntriesLoadInterface interface {
		LoadBookingEntries(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
	}
	type CarClassesToCarModelsLoadInterface interface {
		LoadCarClassesToCarModels(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
	}
	type ResultEntriesLoadInterface interface {
		LoadResultEntries(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
	}
	type SeasonCarClassesLoadInterface interface {
		LoadSeasonCarClasses(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
	}

	return carClassThenLoader[Q]{
		BookingEntries: thenLoadBuilder[Q](
			"BookingEntries",
			func(ctx context.Context, exec bob.Executor, retrieved BookingEntriesLoadInterface, mods ...bob.Mod[*dialect.SelectQuery]) error {
				return retrieved.LoadBookingEntries(ctx, exec, mods...)
			},
		),
		CarClassesToCarModels: thenLoadBuilder[Q](
			"CarClassesToCarModels",
			func(ctx context.Context, exec bob.Executor, retrieved CarClassesToCarModelsLoadInterface, mods ...bob.Mod[*dialect.SelectQuery]) error {
				return retrieved.LoadCarClassesToCarModels(ctx, exec, mods...)
			},
		),
		ResultEntries: thenLoadBuilder[Q](
			"ResultEntries",
			func(ctx context.Context, exec bob.Executor, retrieved ResultEntriesLoadInterface, mods ...bob.Mod[*dialect.SelectQuery]) error {
				return retrieved.LoadResultEntries(ctx, exec, mods...)
			},
		),
		SeasonCarClasses: thenLoadBuilder[Q](
			"SeasonCarClasses",
			func(ctx context.Context, exec bob.Executor, retrieved SeasonCarClassesLoadInterface, mods ...bob.Mod[*dialect.SelectQuery]) error {
				return retrieved.LoadSeasonCarClasses(ctx, exec, mods...)
			},
		),
	}
}

// LoadBookingEntries loads the carClass's BookingEntries into the .R struct
func (o *CarClass) LoadBookingEntries(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}

	// Reset the relationship
	o.R.BookingEntries = nil
	o.R.Loaded.BookingEntries = false

	related, err := o.BookingEntries(mods...).All(ctx, exec)
	if err != nil {
		return err
	}

	for _, rel := range related {
		rel.R.CarClass = o
		rel.R.Loaded.CarClass = true
	}

	o.R.BookingEntries = related
	o.R.Loaded.BookingEntries = true
	return nil
}

// LoadBookingEntries loads the carClass's BookingEntries into the .R struct
func (os CarClassSlice) LoadBookingEntries(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}

	bookingEntries, err := os.BookingEntries(mods...).All(ctx, exec)
	if err != nil {
		return err
	}

	for _, o := range os {
		if o == nil {
			continue
		}

		o.R.BookingEntries = nil
		o.R.Loaded.BookingEntries = true
	}
	// O(N+M) stitch via a map keyed by the join column (key -> []parent; was O(N*M)).
	carClassByKey := make(map[int32][]*CarClass, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}

		carClassByKey[o.ID] = append(carClassByKey[o.ID], o)
	}

	for _, rel := range bookingEntries {

		if !rel.CarClassID.IsValue() {
			continue
		}

		owners, ok := carClassByKey[rel.CarClassID.MustGet()]
		if !ok {
			continue
		}

		for _, o := range owners {

			rel.R.CarClass = o
			rel.R.Loaded.CarClass = true

			o.R.BookingEntries = append(o.R.BookingEntries, rel)

		}
	}

	return nil
}

// LoadCarClassesToCarModels loads the carClass's CarClassesToCarModels into the .R struct
func (o *CarClass) LoadCarClassesToCarModels(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}

	// Reset the relationship
	o.R.CarClassesToCarModels = nil
	o.R.Loaded.CarClassesToCarModels = false

	related, err := o.CarClassesToCarModels(mods...).All(ctx, exec)
	if err != nil {
		return err
	}

	for _, rel := range related {
		rel.R.CarClass = o
		rel.R.Loaded.CarClass = true
	}

	o.R.CarClassesToCarModels = related
	o.R.Loaded.CarClassesToCarModels = true
	return nil
}

// LoadCarClassesToCarModels loads the carClass's CarClassesToCarModels into the .R struct
func (os CarClassSlice) LoadCarClassesToCarModels(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}

	carClassesToCarModels, err := os.CarClassesToCarModels(mods...).All(ctx, exec)
	if err != nil {
		return err
	}

	for _, o := range os {
		if o == nil {
			continue
		}

		o.R.CarClassesToCarModels = nil
		o.R.Loaded.CarClassesToCarModels = true
	}
	// O(N+M) stitch via a map keyed by the join column (key -> []parent; was O(N*M)).
	carClassByKey := make(map[int32][]*CarClass, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}

		carClassByKey[o.ID] = append(carClassByKey[o.ID], o)
	}

	for _, rel := range carClassesToCarModels {

		owners, ok := carClassByKey[rel.CarClassID]
		if !ok {
			continue
		}

		for _, o := range owners {

			rel.R.CarClass = o
			rel.R.Loaded.CarClass = true

			o.R.CarClassesToCarModels = append(o.R.CarClassesToCarModels, rel)

		}
	}

	return nil
}

// LoadResultEntries loads the carClass's ResultEntries into the .R struct
func (o *CarClass) LoadResultEntries(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}

	// Reset the relationship
	o.R.ResultEntries = nil
	o.R.Loaded.ResultEntries = false

	related, err := o.ResultEntries(mods...).All(ctx, exec)
	if err != nil {
		return err
	}

	for _, rel := range related {
		rel.R.CarClass = o
		rel.R.Loaded.CarClass = true
	}

	o.R.ResultEntries = related
	o.R.Loaded.ResultEntries = true
	return nil
}

// LoadResultEntries loads the carClass's ResultEntries into the .R struct
func (os CarClassSlice) LoadResultEntries(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}

	resultEntries, err := os.ResultEntries(mods...).All(ctx, exec)
	if err != nil {
		return err
	}

	for _, o := range os {
		if o == nil {
			continue
		}

		o.R.ResultEntries = nil
		o.R.Loaded.ResultEntries = true
	}
	// O(N+M) stitch via a map keyed by the join column (key -> []parent; was O(N*M)).
	carClassByKey := make(map[int32][]*CarClass, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}

		carClassByKey[o.ID] = append(carClassByKey[o.ID], o)
	}

	for _, rel := range resultEntries {

		if !rel.CarClassID.IsValue() {
			continue
		}

		owners, ok := carClassByKey[rel.CarClassID.MustGet()]
		if !ok {
			continue
		}

		for _, o := range owners {

			rel.R.CarClass = o
			rel.R.Loaded.CarClass = true

			o.R.ResultEntries = append(o.R.ResultEntries, rel)

		}
	}

	return nil
}

// LoadSeasonCarClasses loads the carClass's SeasonCarClasses into the .R struct
func (o *CarClass) LoadSeasonCarClasses(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}

	// Reset the relationship
	o.R.SeasonCarClasses = nil
	o.R.Loaded.SeasonCarClasses = false

	related, err := o.SeasonCarClasses(mods...).All(ctx, exec)
	if err != nil {
		return err
	}

	for _, rel := range related {
		rel.R.CarClass = o
		rel.R.Loaded.CarClass = true
	}

	o.R.SeasonCarClasses = related
	o.R.Loaded.SeasonCarClasses = true
	return nil
}

// LoadSeasonCarClasses loads the carClass's SeasonCarClasses into the .R struct
func (os CarClassSlice) LoadSeasonCarClasses(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}

	seasonCarClasses, err := os.SeasonCarClasses(mods...).All(ctx, exec)
	if err != nil {
		return err
	}

	for _, o := range os {
		if o == nil {
			continue
		}

		o.R.SeasonCarClasses = nil
		o.R.Loaded.SeasonCarClasses = true
	}
	// O(N+M) stitch via a map keyed by the join column (key -> []parent; was O(N*M)).
	carClassByKey := make(map[int32][]*CarClass, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}

		carClassByKey[o.ID] = append(carClassByKey[o.ID], o)
	}

	for _, rel := range seasonCarClasses {

		owners, ok := carClassByKey[rel.CarClassID]
		if !ok {
			continue
		}

		for _, o := range owners {

			rel.R.CarClass = o
			rel.R.Loaded.CarClass = true

			o.R.SeasonCarClasses = append(o.R.SeasonCarClasses, rel)

		}
	}

	return nil
}

// carClassC is where relationship counts are stored.
type carClassC struct {
	BookingEntries        *int64
	CarClassesToCarModels *int64
	ResultEntries         *int64
	SeasonCarClasses      *int64
}

// PreloadCount sets a count in the C struct by name
func (o *CarClass) PreloadCount(name string, count int64) error {
	if o == nil {
		return nil
	}

	switch name {
	case "BookingEntries":
		o.C.BookingEntries = &count
	case "CarClassesToCarModels":
		o.C.CarClassesToCarModels = &count
	case "ResultEntries":
		o.C.ResultEntries = &count
	case "SeasonCarClasses":
		o.C.SeasonCarClasses = &count
	}
	return nil
}

type carClassCountPreloader struct {
	BookingEntries        func(...bob.Mod[*dialect.SelectQuery]) psql.Preloader
	CarClassesToCarModels func(...bob.Mod[*dialect.SelectQuery]) psql.Preloader
	ResultEntries         func(...bob.Mod[*dialect.SelectQuery]) psql.Preloader
	SeasonCarClasses      func(...bob.Mod[*dialect.SelectQuery]) psql.Preloader
}

func buildCarClassCountPreloader() carClassCountPreloader {
	return carClassCountPreloader{
		BookingEntries: func(mods ...bob.Mod[*dialect.SelectQuery]) psql.Preloader {
			return countPreloader[*CarClass]("BookingEntries", func(parent string) bob.Expression {
				// Build a correlated subquery: (SELECT COUNT(*) FROM related WHERE fk = parent.pk)
				if parent == "" {
					parent = CarClasses.Alias()
				}

				subqueryMods := []bob.Mod[*dialect.SelectQuery]{
					sm.Columns(psql.Raw("count(*)")),

					sm.From(BookingEntries.NameAsExpr()),
					sm.Where(psql.Quote(BookingEntries.Alias(), "car_class_id").EQ(psql.Quote(parent, "id"))),
				}
				subqueryMods = append(subqueryMods, mods...)
				return psql.Group(psql.Select(subqueryMods...).Expression)
			})
		},
		CarClassesToCarModels: func(mods ...bob.Mod[*dialect.SelectQuery]) psql.Preloader {
			return countPreloader[*CarClass]("CarClassesToCarModels", func(parent string) bob.Expression {
				// Build a correlated subquery: (SELECT COUNT(*) FROM related WHERE fk = parent.pk)
				if parent == "" {
					parent = CarClasses.Alias()
				}

				subqueryMods := []bob.Mod[*dialect.SelectQuery]{
					sm.Columns(psql.Raw("count(*)")),

					sm.From(CarClassesToCarModels.NameAsExpr()),
					sm.Where(psql.Quote(CarClassesToCarModels.Alias(), "car_class_id").EQ(psql.Quote(parent, "id"))),
				}
				subqueryMods = append(subqueryMods, mods...)
				return psql.Group(psql.Select(subqueryMods...).Expression)
			})
		},
		ResultEntries: func(mods ...bob.Mod[*dialect.SelectQuery]) psql.Preloader {
			return countPreloader[*CarClass]("ResultEntries", func(parent string) bob.Expression {
				// Build a correlated subquery: (SELECT COUNT(*) FROM related WHERE fk = parent.pk)
				if parent == "" {
					parent = CarClasses.Alias()
				}

				subqueryMods := []bob.Mod[*dialect.SelectQuery]{
					sm.Columns(psql.Raw("count(*)")),

					sm.From(ResultEntries.NameAsExpr()),
					sm.Where(psql.Quote(ResultEntries.Alias(), "car_class_id").EQ(psql.Quote(parent, "id"))),
				}
				subqueryMods = append(subqueryMods, mods...)
				return psql.Group(psql.Select(subqueryMods...).Expression)
			})
		},
		SeasonCarClasses: func(mods ...bob.Mod[*dialect.SelectQuery]) psql.Preloader {
			return countPreloader[*CarClass]("SeasonCarClasses", func(parent string) bob.Expression {
				// Build a correlated subquery: (SELECT COUNT(*) FROM related WHERE fk = parent.pk)
				if parent == "" {
					parent = CarClasses.Alias()
				}

				subqueryMods := []bob.Mod[*dialect.SelectQuery]{
					sm.Columns(psql.Raw("count(*)")),

					sm.From(SeasonCarClasses.NameAsExpr()),
					sm.Where(psql.Quote(SeasonCarClasses.Alias(), "car_class_id").EQ(psql.Quote(parent, "id"))),
				}
				subqueryMods = append(subqueryMods, mods...)
				return psql.Group(psql.Select(subqueryMods...).Expression)
			})
		},
	}
}

type carClassCountThenLoader[Q orm.Loadable] struct {
	BookingEntries        func(...bob.Mod[*dialect.SelectQuery]) orm.Loader[Q]
	CarClassesToCarModels func(...bob.Mod[*dialect.SelectQuery]) orm.Loader[Q]
	ResultEntries         func(...bob.Mod[*dialect.SelectQuery]) orm.Loader[Q]
	SeasonCarClasses      func(...bob.Mod[*dialect.SelectQuery]) orm.Loader[Q]
}

func buildCarClassCountThenLoader[Q orm.Loadable]() carClassCountThenLoader[Q] {
	type BookingEntriesCountInterface interface {
		LoadCountBookingEntries(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
	}
	type CarClassesToCarModelsCountInterface interface {
		LoadCountCarClassesToCarModels(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
	}
	type ResultEntriesCountInterface interface {
		LoadCountResultEntries(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
	}
	type SeasonCarClassesCountInterface interface {
		LoadCountSeasonCarClasses(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
	}

	return carClassCountThenLoader[Q]{
		BookingEntries: countThenLoadBuilder[Q](
			"BookingEntries",
			func(ctx context.Context, exec bob.Executor, retrieved BookingEntriesCountInterface, mods ...bob.Mod[*dialect.SelectQuery]) error {
				return retrieved.LoadCountBookingEntries(ctx, exec, mods...)
			},
		),
		CarClassesToCarModels: countThenLoadBuilder[Q](
			"CarClassesToCarModels",
			func(ctx context.Context, exec bob.Executor, retrieved CarClassesToCarModelsCountInterface, mods ...bob.Mod[*dialect.SelectQuery]) error {
				return retrieved.LoadCountCarClassesToCarModels(ctx, exec, mods...)
			},
		),
		ResultEntries: countThenLoadBuilder[Q](
			"ResultEntries",
			func(ctx context.Context, exec bob.Executor, retrieved ResultEntriesCountInterface, mods ...bob.Mod[*dialect.SelectQuery]) error {
				return retrieved.LoadCountResultEntries(ctx, exec, mods...)
			},
		),
		SeasonCarClasses: countThenLoadBuilder[Q](
			"SeasonCarClasses",
			func(ctx context.Context, exec bob.Executor, retrieved SeasonCarClassesCountInterface, mods ...bob.Mod[*dialect.SelectQuery]) error {
				return retrieved.LoadCountSeasonCarClasses(ctx, exec, mods...)
			},
		),
	}
}

// LoadCountBookingEntries loads the count of BookingEntries into the C struct
func (o *CarClass) LoadCountBookingEntries(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}

	count, err := o.BookingEntries(mods...).Count(ctx, exec)
	if err != nil {
		return err
	}

	o.C.BookingEntries = &count
	return nil
}

// LoadCountBookingEntries loads the count of BookingEntries for a slice in a single batch query
func (os CarClassSlice) LoadCountBookingEntries(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}

	// Build the IN arg expression from parent PKs

	pkID := make(pgtypes.Array[int32], 0, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}
		pkID = append(pkID, o.ID)
	}
	PKArgExpr := psql.Any(psql.Cast(psql.Arg(pkID), "integer[]"))

	// countResult holds one scanned row from the batch count query.
	// FK columns are aliased to the parent PK column names for direct map lookup.
	type countResult struct {
		ID    int32
		Count int64
	}

	batchMods := []bob.Mod[*dialect.SelectQuery]{
		// SELECT fk AS parent_pk, count(*)
		sm.Columns(
			BookingEntries.Columns.CarClassID.As("id"),
			psql.Raw("count(*) as count"),
		),
		// Single-hop: FROM related table directly
		sm.From(BookingEntries.NameAsExpr()),

		// WHERE fk IN (parent PKs) — psql single-column FK uses `= ANY(array)` (see PKArgExpr above)
		sm.Where(BookingEntries.Columns.CarClassID.EQ(PKArgExpr)),
		// GROUP BY fk columns
		sm.GroupBy(BookingEntries.Columns.CarClassID),
	}
	batchMods = append(batchMods, mods...)

	results, err := bob.All(ctx, exec,
		psql.Select(batchMods...),
		scan.StructMapper[countResult](),
	)
	if err != nil {
		return err
	}

	// Single-column FK: direct map lookup
	countMap := make(map[int32]int64, len(results))
	for _, r := range results {
		countMap[r.ID] = r.Count
	}
	for _, o := range os {
		if o == nil {
			continue
		}
		count := countMap[o.ID]
		o.C.BookingEntries = &count
	}

	return nil
}

// LoadCountCarClassesToCarModels loads the count of CarClassesToCarModels into the C struct
func (o *CarClass) LoadCountCarClassesToCarModels(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}

	count, err := o.CarClassesToCarModels(mods...).Count(ctx, exec)
	if err != nil {
		return err
	}

	o.C.CarClassesToCarModels = &count
	return nil
}

// LoadCountCarClassesToCarModels loads the count of CarClassesToCarModels for a slice in a single batch query
func (os CarClassSlice) LoadCountCarClassesToCarModels(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}

	// Build the IN arg expression from parent PKs

	pkID := make(pgtypes.Array[int32], 0, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}
		pkID = append(pkID, o.ID)
	}
	PKArgExpr := psql.Any(psql.Cast(psql.Arg(pkID), "integer[]"))

	// countResult holds one scanned row from the batch count query.
	// FK columns are aliased to the parent PK column names for direct map lookup.
	type countResult struct {
		ID    int32
		Count int64
	}

	batchMods := []bob.Mod[*dialect.SelectQuery]{
		// SELECT fk AS parent_pk, count(*)
		sm.Columns(
			CarClassesToCarModels.Columns.CarClassID.As("id"),
			psql.Raw("count(*) as count"),
		),
		// Single-hop: FROM related table directly
		sm.From(CarClassesToCarModels.NameAsExpr()),

		// WHERE fk IN (parent PKs) — psql single-column FK uses `= ANY(array)` (see PKArgExpr above)
		sm.Where(CarClassesToCarModels.Columns.CarClassID.EQ(PKArgExpr)),
		// GROUP BY fk columns
		sm.GroupBy(CarClassesToCarModels.Columns.CarClassID),
	}
	batchMods = append(batchMods, mods...)

	results, err := bob.All(ctx, exec,
		psql.Select(batchMods...),
		scan.StructMapper[countResult](),
	)
	if err != nil {
		return err
	}

	// Single-column FK: direct map lookup
	countMap := make(map[int32]int64, len(results))
	for _, r := range results {
		countMap[r.ID] = r.Count
	}
	for _, o := range os {
		if o == nil {
			continue
		}
		count := countMap[o.ID]
		o.C.CarClassesToCarModels = &count
	}

	return nil
}

// LoadCountResultEntries loads the count of ResultEntries into the C struct
func (o *CarClass) LoadCountResultEntries(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}

	count, err := o.ResultEntries(mods...).Count(ctx, exec)
	if err != nil {
		return err
	}

	o.C.ResultEntries = &count
	return nil
}

// LoadCountResultEntries loads the count of ResultEntries for a slice in a single batch query
func (os CarClassSlice) LoadCountResultEntries(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}

	// Build the IN arg expression from parent PKs

	pkID := make(pgtypes.Array[int32], 0, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}
		pkID = append(pkID, o.ID)
	}
	PKArgExpr := psql.Any(psql.Cast(psql.Arg(pkID), "integer[]"))

	// countResult holds one scanned row from the batch count query.
	// FK columns are aliased to the parent PK column names for direct map lookup.
	type countResult struct {
		ID    int32
		Count int64
	}

	batchMods := []bob.Mod[*dialect.SelectQuery]{
		// SELECT fk AS parent_pk, count(*)
		sm.Columns(
			ResultEntries.Columns.CarClassID.As("id"),
			psql.Raw("count(*) as count"),
		),
		// Single-hop: FROM related table directly
		sm.From(ResultEntries.NameAsExpr()),

		// WHERE fk IN (parent PKs) — psql single-column FK uses `= ANY(array)` (see PKArgExpr above)
		sm.Where(ResultEntries.Columns.CarClassID.EQ(PKArgExpr)),
		// GROUP BY fk columns
		sm.GroupBy(ResultEntries.Columns.CarClassID),
	}
	batchMods = append(batchMods, mods...)

	results, err := bob.All(ctx, exec,
		psql.Select(batchMods...),
		scan.StructMapper[countResult](),
	)
	if err != nil {
		return err
	}

	// Single-column FK: direct map lookup
	countMap := make(map[int32]int64, len(results))
	for _, r := range results {
		countMap[r.ID] = r.Count
	}
	for _, o := range os {
		if o == nil {
			continue
		}
		count := countMap[o.ID]
		o.C.ResultEntries = &count
	}

	return nil
}

// LoadCountSeasonCarClasses loads the count of SeasonCarClasses into the C struct
func (o *CarClass) LoadCountSeasonCarClasses(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}

	count, err := o.SeasonCarClasses(mods...).Count(ctx, exec)
	if err != nil {
		return err
	}

	o.C.SeasonCarClasses = &count
	return nil
}

// LoadCountSeasonCarClasses loads the count of SeasonCarClasses for a slice in a single batch query
func (os CarClassSlice) LoadCountSeasonCarClasses(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}

	// Build the IN arg expression from parent PKs

	pkID := make(pgtypes.Array[int32], 0, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}
		pkID = append(pkID, o.ID)
	}
	PKArgExpr := psql.Any(psql.Cast(psql.Arg(pkID), "integer[]"))

	// countResult holds one scanned row from the batch count query.
	// FK columns are aliased to the parent PK column names for direct map lookup.
	type countResult struct {
		ID    int32
		Count int64
	}

	batchMods := []bob.Mod[*dialect.SelectQuery]{
		// SELECT fk AS parent_pk, count(*)
		sm.Columns(
			SeasonCarClasses.Columns.CarClassID.As("id"),
			psql.Raw("count(*) as count"),
		),
		// Single-hop: FROM related table directly
		sm.From(SeasonCarClasses.NameAsExpr()),

		// WHERE fk IN (parent PKs) — psql single-column FK uses `= ANY(array)` (see PKArgExpr above)
		sm.Where(SeasonCarClasses.Columns.CarClassID.EQ(PKArgExpr)),
		// GROUP BY fk columns
		sm.GroupBy(SeasonCarClasses.Columns.CarClassID),
	}
	batchMods = append(batchMods, mods...)

	results, err := bob.All(ctx, exec,
		psql.Select(batchMods...),
		scan.StructMapper[countResult](),
	)
	if err != nil {
		return err
	}

	// Single-column FK: direct map lookup
	countMap := make(map[int32]int64, len(results))
	for _, r := range results {
		countMap[r.ID] = r.Count
	}
	for _, o := range os {
		if o == nil {
			continue
		}
		count := countMap[o.ID]
		o.C.SeasonCarClasses = &count
	}

	return nil
}

type carClassJoins[Q dialect.Joinable] struct {
	typ                   string
	BookingEntries        modAs[Q, bookingEntryColumns]
	CarClassesToCarModels modAs[Q, carClassesToCarModelColumns]
	ResultEntries         modAs[Q, resultEntryColumns]
	SeasonCarClasses      modAs[Q, seasonCarClassColumns]
}

func (j carClassJoins[Q]) aliasedAs(alias string) carClassJoins[Q] {
	return buildCarClassJoins[Q](buildCarClassColumns(alias), j.typ)
}

func buildCarClassJoins[Q dialect.Joinable](cols carClassColumns, typ string) carClassJoins[Q] {
	return carClassJoins[Q]{
		typ: typ,
		BookingEntries: modAs[Q, bookingEntryColumns]{
			c: BookingEntries.Columns,
			f: func(to bookingEntryColumns) bob.Mod[Q] {
				mods := make(mods.QueryMods[Q], 0, 1)

				{
					mods = append(mods, dialect.Join[Q](typ, BookingEntries.NameExpr().As(to.Alias())).On(
						to.CarClassID.EQ(cols.ID),
					))
				}

				return mods
			},
		},
		CarClassesToCarModels: modAs[Q, carClassesToCarModelColumns]{
			c: CarClassesToCarModels.Columns,
			f: func(to carClassesToCarModelColumns) bob.Mod[Q] {
				mods := make(mods.QueryMods[Q], 0, 1)

				{
					mods = append(mods, dialect.Join[Q](typ, CarClassesToCarModels.NameExpr().As(to.Alias())).On(
						to.CarClassID.EQ(cols.ID),
					))
				}

				return mods
			},
		},
		ResultEntries: modAs[Q, resultEntryColumns]{
			c: ResultEntries.Columns,
			f: func(to resultEntryColumns) bob.Mod[Q] {
				mods := make(mods.QueryMods[Q], 0, 1)

				{
					mods = append(mods, dialect.Join[Q](typ, ResultEntries.NameExpr().As(to.Alias())).On(
						to.CarClassID.EQ(cols.ID),
					))
				}

				return mods
			},
		},
		SeasonCarClasses: modAs[Q, seasonCarClassColumns]{
			c: SeasonCarClasses.Columns,
			f: func(to seasonCarClassColumns) bob.Mod[Q] {
				mods := make(mods.QueryMods[Q], 0, 1)

				{
					mods = append(mods, dialect.Join[Q](typ, SeasonCarClasses.NameExpr().As(to.Alias())).On(
						to.CarClassID.EQ(cols.ID),
					))
				}

				return mods
			},
		},
	}
}
