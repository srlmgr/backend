...
func TestRacingSimUniqueConstraintErrors(t *testing.T) {
	if testDB == nil {
	...
	tests := []struct {
		...
		conflictMods func(context.Context, *testing.T, bob.Executor, *models.RacingSim) factory.RacingSimModSlice
	}{
		{
			name:        "ErrUniqueRacingSimsPkey",
			expectedErr: RacingSimErrors.ErrUniqueRacingSimsPkey,
			conflictMods: func(ctx context.Context, t *testing.T, exec bob.Executor, obj *models.RacingSim) factory.RacingSimModSlice {
				...
				return factory.RacingSimModSlice{
					factory.RacingSimMods.ID(obj.ID),
				}
			},
		},
		{
			...
			conflictMods: func(ctx context.Context, t *testing.T, exec bob.Executor, obj *models.RacingSim) factory.RacingSimModSlice {
				...
				return factory.RacingSimModSlice{
					factory.RacingSimMods.Name(obj.Name),
				}
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			obj, err := f.NewRacingSimWithContext(ctx, factory.RacingSimMods.WithParentsCascading()).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			obj2, err := f.NewRacingSimWithContext(ctx).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			err = obj2.Update(ctx, exec, f.NewRacingSimWithContext(ctx, tt.conflictMods(ctx, t, exec, obj)...).BuildSetter())
