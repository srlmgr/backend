...
func TestTeamUniqueConstraintErrors(t *testing.T) {
	if testDB == nil {
	...
	tests := []struct {
		...
		conflictMods func(context.Context, *testing.T, bob.Executor, *models.Team) factory.TeamModSlice
	}{
		{
			name:        "ErrUniqueTeamsPkey",
			expectedErr: TeamErrors.ErrUniqueTeamsPkey,
		...
		},
		{
			...
			conflictMods: func(ctx context.Context, t *testing.T, exec bob.Executor, obj *models.Team) factory.TeamModSlice {
				shouldUpdate := false
				updateMods := make(factory.TeamModSlice, 0, 2)
				...
				return factory.TeamModSlice{
					factory.TeamMods.SeasonID(obj.SeasonID),
					factory.TeamMods.Name(obj.Name),
				}
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			obj, err := f.NewTeamWithContext(ctx, factory.TeamMods.WithParentsCascading()).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			obj2, err := f.NewTeamWithContext(ctx).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			err = obj2.Update(ctx, exec, f.NewTeamWithContext(ctx, tt.conflictMods(ctx, t, exec, obj)...).BuildSetter())
