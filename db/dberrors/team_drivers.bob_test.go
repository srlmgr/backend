...
func TestTeamDriverUniqueConstraintErrors(t *testing.T) {
	if testDB == nil {
	...
	tests := []struct {
		...
		conflictMods func(context.Context, *testing.T, bob.Executor, *models.TeamDriver) factory.TeamDriverModSlice
	}{
		{
			name:        "ErrUniqueTeamDriversPkey",
			expectedErr: TeamDriverErrors.ErrUniqueTeamDriversPkey,
			conflictMods: func(ctx context.Context, t *testing.T, exec bob.Executor, obj *models.TeamDriver) factory.TeamDriverModSlice {
				...
				return factory.TeamDriverModSlice{
					factory.TeamDriverMods.ID(obj.ID),
				}
			},
		},
		{
		...
		},
		{
			...
			conflictMods: func(ctx context.Context, t *testing.T, exec bob.Executor, obj *models.TeamDriver) factory.TeamDriverModSlice {
				shouldUpdate := false
				updateMods := make(factory.TeamDriverModSlice, 0, 3)
				...
				return factory.TeamDriverModSlice{
					factory.TeamDriverMods.TeamID(obj.TeamID),
					factory.TeamDriverMods.DriverID(obj.DriverID),
					factory.TeamDriverMods.JoinedAt(obj.JoinedAt),
				}
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			obj, err := f.NewTeamDriverWithContext(ctx, factory.TeamDriverMods.WithParentsCascading()).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			obj2, err := f.NewTeamDriverWithContext(ctx).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			err = obj2.Update(ctx, exec, f.NewTeamDriverWithContext(ctx, tt.conflictMods(ctx, t, exec, obj)...).BuildSetter())
