...
func TestEventUniqueConstraintErrors(t *testing.T) {
	if testDB == nil {
	...
	tests := []struct {
		...
		conflictMods func(context.Context, *testing.T, bob.Executor, *models.Event) factory.EventModSlice
	}{
		{
			name:        "ErrUniqueEventsPkey",
			expectedErr: EventErrors.ErrUniqueEventsPkey,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			obj, err := f.NewEventWithContext(ctx, factory.EventMods.WithParentsCascading()).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			obj2, err := f.NewEventWithContext(ctx).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			err = obj2.Update(ctx, exec, f.NewEventWithContext(ctx, tt.conflictMods(ctx, t, exec, obj)...).BuildSetter())
