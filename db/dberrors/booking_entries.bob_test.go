...
func TestBookingEntryUniqueConstraintErrors(t *testing.T) {
	if testDB == nil {
	...
	tests := []struct {
		...
		conflictMods func(context.Context, *testing.T, bob.Executor, *models.BookingEntry) factory.BookingEntryModSlice
	}{
		{
			name:        "ErrUniqueBookingEntriesPkey",
			expectedErr: BookingEntryErrors.ErrUniqueBookingEntriesPkey,
			conflictMods: func(ctx context.Context, t *testing.T, exec bob.Executor, obj *models.BookingEntry) factory.BookingEntryModSlice {
				...
				return factory.BookingEntryModSlice{
					factory.BookingEntryMods.ID(obj.ID),
				}
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			obj, err := f.NewBookingEntryWithContext(ctx, factory.BookingEntryMods.WithParentsCascading()).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			obj2, err := f.NewBookingEntryWithContext(ctx).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			err = obj2.Update(ctx, exec, f.NewBookingEntryWithContext(ctx, tt.conflictMods(ctx, t, exec, obj)...).BuildSetter())
