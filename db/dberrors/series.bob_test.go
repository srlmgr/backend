...
func TestSeriesUniqueConstraintErrors(t *testing.T) {
	if testDB == nil {
	...
	tests := []struct {
		...
		conflictMods func(context.Context, *testing.T, bob.Executor, *models.Series) factory.SeriesModSlice
	}{
		{
			name:        "ErrUniqueSeriesPkey",
			expectedErr: SeriesErrors.ErrUniqueSeriesPkey,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			obj, err := f.NewSeriesWithContext(ctx, factory.SeriesMods.WithParentsCascading()).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			obj2, err := f.NewSeriesWithContext(ctx).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			err = obj2.Update(ctx, exec, f.NewSeriesWithContext(ctx, tt.conflictMods(ctx, t, exec, obj)...).BuildSetter())
