// Code generated by BobGen psql v0.48.0. DO NOT EDIT.
// This file is meant to be re-generated in place and/or deleted at any time.

package dbinfo

import "github.com/aarondl/opt/null"

var Teams = Table[
	teamColumns,
	teamIndexes,
	teamForeignKeys,
	teamUniques,
	teamChecks,
]{
	Schema: "",
	Name:   "teams",
	Columns: teamColumns{
		ID: column{
			Name:      "id",
			DBType:    "integer",
			Default:   "nextval('teams_id_seq'::regclass)",
			Comment:   "",
			Nullable:  false,
			Generated: false,
			AutoIncr:  false,
		},
		SeasonID: column{
			Name:      "season_id",
			DBType:    "integer",
			Default:   "",
			Comment:   "",
			Nullable:  false,
			Generated: false,
			AutoIncr:  false,
		},
		Name: column{
			Name:      "name",
			DBType:    "text",
			Default:   "",
			Comment:   "",
			Nullable:  false,
			Generated: false,
			AutoIncr:  false,
		},
		IsActive: column{
			Name:      "is_active",
			DBType:    "boolean",
			Default:   "true",
			Comment:   "",
			Nullable:  false,
			Generated: false,
			AutoIncr:  false,
		},
		CarModelVariantID: column{
			Name:      "car_model_variant_id",
			DBType:    "integer",
			Default:   "NULL",
			Comment:   "",
			Nullable:  true,
			Generated: false,
			AutoIncr:  false,
		},
		CarNumber: column{
			Name:      "car_number",
			DBType:    "text",
			Default:   "NULL",
			Comment:   "",
			Nullable:  true,
			Generated: false,
			AutoIncr:  false,
		},
		JoinedAt: column{
			Name:      "joined_at",
			DBType:    "timestamp with time zone",
			Default:   "now()",
			Comment:   "",
			Nullable:  false,
			Generated: false,
			AutoIncr:  false,
		},
		LeftAt: column{
			Name:      "left_at",
			DBType:    "timestamp with time zone",
			Default:   "NULL",
			Comment:   "",
			Nullable:  true,
			Generated: false,
			AutoIncr:  false,
		},
		CreatedAt: column{
			Name:      "created_at",
			DBType:    "timestamp with time zone",
			Default:   "now()",
			Comment:   "",
			Nullable:  false,
			Generated: false,
			AutoIncr:  false,
		},
		UpdatedAt: column{
			Name:      "updated_at",
			DBType:    "timestamp with time zone",
			Default:   "now()",
			Comment:   "",
			Nullable:  false,
			Generated: false,
			AutoIncr:  false,
		},
		CreatedBy: column{
			Name:      "created_by",
			DBType:    "text",
			Default:   "'system'::text",
			Comment:   "",
			Nullable:  false,
			Generated: false,
			AutoIncr:  false,
		},
		UpdatedBy: column{
			Name:      "updated_by",
			DBType:    "text",
			Default:   "'system'::text",
			Comment:   "",
			Nullable:  false,
			Generated: false,
			AutoIncr:  false,
		},
	},
	Indexes: teamIndexes{
		TeamsPkey: index{
			Type: "btree",
			Name: "teams_pkey",
			Columns: []indexColumn{
				{
					Name:         "id",
					Desc:         null.FromCond(false, true),
					IsExpression: false,
				},
			},
			Unique:        true,
			Comment:       "",
			NullsFirst:    []bool{false},
			NullsDistinct: false,
			Where:         "",
			Include:       []string{},
		},
		IdxTeamsSeasonID: index{
			Type: "btree",
			Name: "idx_teams_season_id",
			Columns: []indexColumn{
				{
					Name:         "season_id",
					Desc:         null.FromCond(false, true),
					IsExpression: false,
				},
			},
			Unique:        false,
			Comment:       "",
			NullsFirst:    []bool{false},
			NullsDistinct: false,
			Where:         "",
			Include:       []string{},
		},
	},
	PrimaryKey: &constraint{
		Name:    "teams_pkey",
		Columns: []string{"id"},
		Comment: "",
	},
	ForeignKeys: teamForeignKeys{
		TeamsTeamsCarModelVariantIDFK: foreignKey{
			constraint: constraint{
				Name:    "teams.teams_car_model_variant_id_fk",
				Columns: []string{"car_model_variant_id"},
				Comment: "",
			},
			ForeignTable:   "car_model_variants",
			ForeignColumns: []string{"id"},
		},
		TeamsTeamsSeasonIDFK: foreignKey{
			constraint: constraint{
				Name:    "teams.teams_season_id_fk",
				Columns: []string{"season_id"},
				Comment: "",
			},
			ForeignTable:   "seasons",
			ForeignColumns: []string{"id"},
		},
	},

	Checks: teamChecks{
		TeamsLeftAtCheck: check{
			constraint: constraint{
				Name:    "teams_left_at_check",
				Columns: []string{"left_at", "joined_at"},
				Comment: "",
			},
			Expression: "((left_at IS NULL) OR (left_at >= joined_at))",
		},
	},
	Comment: "",
}

type teamColumns struct {
	ID                column
	SeasonID          column
	Name              column
	IsActive          column
	CarModelVariantID column
	CarNumber         column
	JoinedAt          column
	LeftAt            column
	CreatedAt         column
	UpdatedAt         column
	CreatedBy         column
	UpdatedBy         column
}

func (c teamColumns) AsSlice() []column {
	return []column{
		c.ID, c.SeasonID, c.Name, c.IsActive, c.CarModelVariantID, c.CarNumber, c.JoinedAt, c.LeftAt, c.CreatedAt, c.UpdatedAt, c.CreatedBy, c.UpdatedBy,
	}
}

type teamIndexes struct {
	TeamsPkey        index
	IdxTeamsSeasonID index
}

func (i teamIndexes) AsSlice() []index {
	return []index{
		i.TeamsPkey, i.IdxTeamsSeasonID,
	}
}

type teamForeignKeys struct {
	TeamsTeamsCarModelVariantIDFK foreignKey
	TeamsTeamsSeasonIDFK          foreignKey
}

func (f teamForeignKeys) AsSlice() []foreignKey {
	return []foreignKey{
		f.TeamsTeamsCarModelVariantIDFK, f.TeamsTeamsSeasonIDFK,
	}
}

type teamUniques struct{}

func (u teamUniques) AsSlice() []constraint {
	return []constraint{}
}

type teamChecks struct {
	TeamsLeftAtCheck check
}

func (c teamChecks) AsSlice() []check {
	return []check{
		c.TeamsLeftAtCheck,
	}
}
